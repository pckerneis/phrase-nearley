...
fill <bar baz> with "foobar"`).expanded;
    ...
    it("should expand macro with target parameter in assertion", () => {
      const result = parse(`
in order to check $foo has name {name}:
  expect <$foo> to have text "name is {name}"

check <bar baz> has name "bob"`).expanded;
